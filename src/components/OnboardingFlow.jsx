...
const OnboardingFlow = () => {
  ...
  return (
    <div className="min-h-screen bg-gray-50 flex items-center justify-center p-4">
            ...
            {selectedRoles.includes('custom') && (
              <div className="mb-8 p-6 bg-gray-50 rounded-xl">
                ...
                <p className="text-xs text-gray-500 mt-2">
                  {customRole.length}/50 characters • This will be visible to other team members
                </p>
              </div>
            )}
                    ...
                    <input 
                      ...
                      onChange={(e) => {
                        ...
                        if (file) {
                          ...
                          reader.onload = (e) => setProfileData(prev => ({ ...prev, photo: e.target.result }));
      ...
      {/* Screen 3: Setup Complete */}
      {currentScreen === 3 && (
        <div className="bg-white rounded-2xl shadow-xl w-full max-w-lg overflow-hidden text-center p-8">
          <div className="mb-6">
            <div className="w-20 h-20 bg-green-100 rounded-full flex items-center justify-center mx-auto mb-4 animate-pulse">
              <Check className="w-10 h-10 text-green-600" />
            </div>
            <h2 className="text-2xl font-bold text-gray-900 mb-2">Welcome to Chatter!</h2>
      ...
      <div className="fixed bottom-4 right-4 bg-white rounded-lg shadow-lg p-4 border">
        <div className="flex gap-2">
          <button
            onClick={() => setCurrentScreen(1)}
            className={`px-3 py-1 rounded text-sm ${currentScreen === 1 ? 'bg-indigo-600 text-white' : 'bg-gray-200'}`}
          >
            Screen 1
          </button>
          <button
            onClick={() => setCurrentScreen(2)}
            className={`px-3 py-1 rounded text-sm ${currentScreen === 2 ? 'bg-indigo-600 text-white' : 'bg-gray-200'}`}
          >
            Screen 2
          </button>
          <button
            onClick={() => setCurrentScreen(3)}
            className={`px-3 py-1 rounded text-sm ${currentScreen === 3 ? 'bg-indigo-600 text-white' : 'bg-gray-200'}`}
          >
            Screen 3
          </button>
        </div>
